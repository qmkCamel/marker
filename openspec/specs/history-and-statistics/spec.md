# history-and-statistics Specification

## Purpose
定义本地追踪项的历史浏览与基础统计能力，使用户可以回看每日记录并获得稳定的反馈摘要。
## Requirements
...
系统必须（MUST）提供基础统计反馈，包括总追踪项数、累计记录数、当前连胜和近 7/30 天完成率中的至少一组可见指标。

#### Scenario: 用户打开统计视图
- **WHEN** 用户进入统计视图
- **THEN** 系统显示基于本地记录数据计算得到的统计摘要
- **AND** 这些统计结果会随着记录数据变化而更新
